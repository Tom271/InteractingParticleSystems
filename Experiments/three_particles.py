...
default_parameters = {
    ...
    "initial_dist_v": np.array([1.5, 1, 0]),
    "dt": 0.01,
    "T_end": 50,
...
# gammas = [0.0,0.02,0.04,0.06,0.08,0.1,0.2, 0.5 ,1.0]
...
# fig.text(0.06, 0.5, r"$\bar{v}$", ha='center', va='center', rotation='vertical')
#
...
#             avg_vel_data[j],
#             label=r"${0:.2f}$".format(gammas[j]),
#         )
# #
# conv_to = np.sign(avg_velocity(v)[0])
# ax.plot([0, T], [conv_to, conv_to], "g--")
# ax.plot([0, T], [conv_to + eps, conv_to + eps], "g--")
# ax.plot([0, T], [conv_to - eps, conv_to - eps], "g--")
# # ax.plot(t[: len(avg_velocity(v))], avg_velocity(v), "k--", label="0.5")
# ax.set_xlabel(r"$t$")
# ax.set_ylabel(r"$\bar{v}$")
# ax.legend(title=r"$\gamma$", loc="right")
# # plt.savefig("threepart_avg_vel.eps", transparent=True, format="eps")
# fig.savefig('avg_vel.eps', format='eps')
# plt.show()
#
# #
# fig,ax = plt.subplots()
...
writer = animation.FFMpegWriter(fps=10, extra_args=["-vcodec", "libx264"], bitrate=-1)
plt.show()
ani.save("3particleperiodicgamma04ani.mp4", writer=writer, dpi=600)
