...
def plot_figure_4():
    ...
    print("...slowly ...")
    ax1 = sns.kdeplot(np.repeat(t[:int(20//dt)],particle_count),x[:int(20//dt),].flatten(),shade=True, cmap=sns.cubehelix_palette(25,as_cmap=True))
    ax1.set(xlabel='Time', ylabel='Position', xlim=(0,20), ylim=(0, length),title="First 20s KDE")
    #TODO: Change marker size and shape, maybe swap for plot
    ax[0,1].scatter(np.repeat(t[:int(20//dt), np.newaxis], 50, axis=1), x[:int(20//dt), 0:50],marker=".", s=1)
    ax[0,1].set(xlabel='Time', ylabel='Position', xlim=(0,20), ylim=(0, length),title="First 20s Traj")
    ...
    ax2.set(xlabel='Time', ylabel='Position',xlim=(T_final - 20, T_final),ylim=(0, length), title="Last 20s KDE")
    #TODO: Change marker size and shape
    ax[1,1].scatter(np.repeat(t[-int(20//dt):, np.newaxis], 50, axis=1), x[-int(20//dt):, 0:50], marker=".", s=1)
    ax[1,1].set(xlabel='Time', ylabel='Position', xlim=(80,100), ylim=(0,length),title="Last 20s Traj")
