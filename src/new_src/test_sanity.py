...
def test_zeros():
    ...
    assert calculate_interaction(x,v, phi_zero, L, interaction_vector).all() == 0.
...
def test_OU():
    ...
    ax[1].hist(v[-500:-1,].flatten(), bins=np.arange(v.min(), v.max(), 0.15),
                                          density=True)
    ax[1].hist(v_hom[-500:-1,].flatten(), bins=np.arange(v.min(), v.max(), 0.15),
                                        density=True, alpha = 0.3)
    ax[1].plot(np.arange(-v.max(),v.max(),0.01), stats.norm.pdf(np.arange(-v.max(),v.max(),0.01), loc=0, scale=np.sqrt(diffusion)), '--')
    ...
    print("L2 discrepancy of space distribution:", CL2(x[-100:,].flatten()), ", expected is ", (1/particles * (5/4 - 13/12)))
    plt.show()
...
def test_normal():
    ...
    diffusion = 4
    startTime = datetime.now()
    t,x,v = run_full_particle_system(interaction_function="Uniform", herding_function="Step",
            particles=particles,
            D=diffusion,
            initial_dist_v=np.random.normal(loc=-0.5, scale=np.sqrt(2), size=particles),
            T_end=100,
    ...
    ax[1].hist(v_hom[-500:-1,].flatten(), bins=np.arange(v.min(), v.max(), 0.15),
                                        density=True, alpha = 0.3)
    ax[1].plot(np.arange(-v.max(),v.max(),0.01), stats.norm.pdf(np.arange(-v.max(),v.max(),0.01), loc=-1, scale=np.sqrt(diffusion)), '--')
    ax[1].set(xlabel='Velocity')

    true_prob_x = 1/(2*np.pi)*np.ones(len(model_prob_x))
    true_prob_v = stats.norm.pdf(np.arange(v.min(), v.max()-0.15, 0.15), loc=1, scale=np.sqrt(diffusion))
    fig.savefig('xvhistmeanone.jpg', format='jpg', dpi=1000)
    print("KL Divergence of velocity distribution:",     stats.entropy(model_prob_v, true_prob_v))

    print("L2 discrepancy of space distribution:", CL2(x[-20:,].flatten()), ", expected is ", (1/particles * (5/4 - 13/12)))
    plt.show()
...
def test_Garnier():
    ...
    startTime = datetime.now()
    t,x,v = run_full_particle_system(interaction_function="Garnier", herding_function="Garneir",
            particles=particles,
            D=diffusion,
            initial_dist_v=np.random.normal(loc=0.5, scale=np.sqrt(2), size=particles),
            T_end=100,
