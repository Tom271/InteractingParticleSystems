...
def plot_torus(time_point, t, x, v, ax):
    ...
    pos_vel = [
        x[time_point, idx] if vel >= 0 else None
        for idx, vel in enumerate(v[time_point, :])
    ]
    ...
    (neg_points,) = ax.plot(
        np.sin(neg_vel), np.cos(neg_vel), linestyle="None", marker="o"
    )
    (pos_points,) = ax.plot(
        np.sin(pos_vel), np.cos(pos_vel), linestyle="None", marker="o"
    )
...
def anim_full(t, x, v, framestep=1):
    ...
    (neg_points,) = big_ax.plot(
        np.sin(neg_vel), np.cos(neg_vel), linestyle="None", marker="o", alpha=0.8, ms=10
    )
    (pos_points,) = big_ax.plot(
        np.sin(pos_vel), np.cos(pos_vel), linestyle="None", marker="o", alpha=0.8, ms=8
    )
    ...
    n_v, bins_v, patches_v = vel_ax.hist(
        v[0, :], bins=np.arange(v.min(), v.max(), 0.15), density=True, label="Velocity"
    )
