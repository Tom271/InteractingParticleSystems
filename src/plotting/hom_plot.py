...
def anim_hist_moments(t, x, m1, var, mu=0, D=1, fs=1, animate=True, timeavg=False):
    ...
    n_x, bins_x, patches_x = pos_ax.hist(
        x[-1,], bins=np.arange(x.min(), x.max(), 0.15), density=True, label="Position"
    )
    ...
    if animate:
        ...
        def animate(i, fs):
            ...
            if timeavg:
                ...
                n_x, _ = np.histogram(
                    x[i * fs,], bins=np.arange(x.min(), x.max(), 0.15), density=True
                )
            if i == 0:
                pos_ax.set_ylim(0, max(stat_dist.max(), n_x.max()) + 0.1)
...
def animate_PDE_hist(t, v, traj, sol):
    ...
    def animate(i):
        ...
        n_v, _ = np.histogram(
            traj[i, :], bins=np.arange(traj.min(), traj.max(), 0.15), density=True
        )
        # Update vel data
